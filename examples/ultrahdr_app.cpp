...
class UltraHdrAppInput {
 public:
  UltraHdrAppInput(const char* hdrIntentRawFile, const char* sdrIntentRawFile,
                   ...
                   bool enableMultiChannelGainMap = false, float maxDispMasteringLuminance = -1.0f)
      : mHdrIntentRawFile(hdrIntentRawFile),
        ...
        mMaxDispMasteringLuminance(maxDispMasteringLuminance),
        mMode(0){};
  ...
  UltraHdrAppInput(const char* uhdrFile, const char* outputFile,
      ...
      : mHdrIntentRawFile(nullptr),
        ...
        mMaxDispMasteringLuminance(-1.0f),
        mMode(1){};
  ...
  const float mMaxDispMasteringLuminance;
  const int mMode;
...
#define RET_IF_ERR(x)                            \
  ...
  if (mMaxDispMasteringLuminance != -1.0f) {
    RET_IF_ERR(uhdr_enc_set_max_display_luminance_nits(handle, mMaxDispMasteringLuminance))
  }
...
static void usage(const char* name) {
  ...
  fprintf(stderr, "    -L    maximum display mastering luminance in nits. [600 - 10000] \n");
  fprintf(stderr, "\n## decoder options : \n");
...
int main(int argc, char* argv[]) {
  char opt_string[] = "p:y:i:g:f:w:h:C:c:t:q:o:O:m:j:e:a:b:z:R:s:M:Q:L:";
  ...
  float max_display_mastering_luminance = -1.0f;
  int compute_psnr = 0;
  int ch;
  while ((ch = getopt_s(argc, argv, opt_string)) != -1) {
    switch (ch) {
      ...
      case 'z':
        ...
        break;
      case 'L':
        max_display_mastering_luminance = atof(optarg_s);
        break;
      default:
        usage(argv[0]);
        return -1;
  ...
  if (mode == 0) {
    ...
    UltraHdrAppInput appInput(
        ...
        gainmap_compressed_file, gainmap_metadata_cfg_file, output_file ? output_file : "out.jpeg",
        width, height, hdr_cf, sdr_cf, hdr_cg, sdr_cg, hdr_tf, quality, out_tf, out_cf,
        use_full_range_color_hdr, gainmap_scale_factor, gainmap_compression_quality,
        use_multi_channel_gainmap, max_display_mastering_luminance);
