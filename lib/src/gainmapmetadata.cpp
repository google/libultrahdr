...
status_t gain_map_metadata::encodeGainmapMetadata(const gain_map_metadata* metadata,
  ...
  for (int c = 0; c < channelCount; ++c) {
    if (metadata->gainMapMinD[c] != denom ||
...
status_t gain_map_metadata::gainmapMetadataFractionToFloat(const gain_map_metadata* from,
  ...
  to->maxContentBoost = from->gainMapMaxN[0] / from->gainMapMaxD[0];
  to->minContentBoost = from->gainMapMinN[0] / from->gainMapMinD[0];
  ...
  to->hdrCapacityMax = from->alternateHdrHeadroomN / from->alternateHdrHeadroomD;
  to->hdrCapacityMin = from->baseHdrHeadroomN / from->baseHdrHeadroomD;
...
status_t gain_map_metadata::gainmapMetadataFloatToFraction(const ultrahdr_metadata_ptr from,
  ...
  floatToUnsignedFraction(from->hdrCapacityMax, &to->alternateHdrHeadroomN, &to->alternateHdrHeadroomD);
