...
uhdr_error_info_t uhdr_encode(uhdr_codec_private_t* enc) {
  ...
  if (handle->m_effects.size() != 0) {
    status.error_code = UHDR_CODEC_INVALID_PARAM;
    status.has_detail = 1;
    snprintf(status.detail, sizeof status.detail, "image transforms are not currently enabled");
    return status;
  }
...
uhdr_error_info_t uhdr_add_transform_mirror_horz(uhdr_codec_private_t* codec) {
  uhdr_error_info_t status = g_no_error;
  ...
  codec->m_effects.push_back(new ultrahdr::uhdr_mirror_effect_t(ultrahdr::UHDR_MIRROR_HORIZONTAL));

  return status;
}

uhdr_error_info_t uhdr_add_transform_mirror_vert(uhdr_codec_private_t* codec) {
  uhdr_error_info_t status = g_no_error;
  ...
  codec->m_effects.push_back(new ultrahdr::uhdr_mirror_effect_t(ultrahdr::UHDR_MIRROR_VERTICAL));

  return status;
}

uhdr_error_info_t uhdr_add_transform_rotate90(uhdr_codec_private_t* codec) {
  uhdr_error_info_t status = g_no_error;
  ...
  codec->m_effects.push_back(new ultrahdr::uhdr_rotate_effect_t(90));

  return status;
}

uhdr_error_info_t uhdr_add_transform_rotate180(uhdr_codec_private_t* codec) {
  uhdr_error_info_t status = g_no_error;
  ...
  codec->m_effects.push_back(new ultrahdr::uhdr_rotate_effect_t(180));

  return status;
}

uhdr_error_info_t uhdr_add_transform_rotate270(uhdr_codec_private_t* codec) {
  uhdr_error_info_t status = g_no_error;
  ...
  codec->m_effects.push_back(new ultrahdr::uhdr_rotate_effect_t(270));

  return status;
}

uhdr_error_info_t uhdr_add_transform_crop(uhdr_codec_private_t* codec, int left, int right, int top,
                                          int bottom) {
  uhdr_error_info_t status = g_no_error;
...
uhdr_error_info_t uhdr_add_transform_resize(uhdr_codec_private_t* codec, int width, int height) {
  uhdr_error_info_t status = g_no_error;
