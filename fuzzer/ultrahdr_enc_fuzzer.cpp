...
void UltraHdrEncFuzzer::process() {
  if (mFdp.remaining_bytes()) {
    ...
    // mastering display peak brightness
    auto masteringDispPeakBrightness = mFdp.ConsumeFloatingPointInRange<float>(100.0f, 10500.0f);
...
#define ON_ERR(x)                              \
  {                                            \
    ...
    ON_ERR(uhdr_enc_set_mastering_display_peak_brightness(enc_handle, masteringDispPeakBrightness))
